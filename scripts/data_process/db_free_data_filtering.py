...
import numpy as np
from db_free_reverse_engineering import reverse_episode_log
...
def get_episodes_by_env_dict(
    tags: list,
    scenarios: set,
) -> dict[str, list[FakeEpisodeLog]]:
    # Load all episodes from tags
    eps_by_tag = {}
    for tag in tags:
        # eps = FakeEpisodeLog.find(FakeEpisodeLog.tag == tag).all()
        eps = [ep for ep in EPISODES if ep.tag == tag]
        if len(eps) > 0:
            eps_by_tag[tag] = eps
...
def behavior_cloning_data_filtering_gpt4_gpt4():
    ...
    with open("sft_round_1_bc_data_top_2.json", "w") as f:
