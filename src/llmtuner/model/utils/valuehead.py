...
def configure_valuehead(config: "PretrainedConfig") -> None:
    if getattr(config, "model_type", None) == "llava":
        setattr(
            config,
            "hidden_size",
            getattr(config.vision_config, "intermediate_size", None),
        )
...
def load_valuehead_params(
    ...
    try:
        ...
        logger.info(
            "Failed to load {}: {}".format(V_HEAD_SAFE_WEIGHTS_NAME, str(err))
        )
    ...
    try:
        ...
        logger.info(
            "Failed to load {}: {}".format(V_HEAD_WEIGHTS_NAME, str(err))
        )
