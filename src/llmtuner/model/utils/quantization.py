...
def _get_quantization_dataset(
    ...
    for _ in range(model_args.export_quantization_nsamples):
        ...
        samples.append(
            tokenizer.decode(input_ids[0].tolist(), skip_special_tokens=True)
        )
...
def configure_quantization(
    ...
    if getattr(config, "quantization_config", None):  # ptq
        ...
        require_version(
            "optimum>=1.16.0", "To fix: pip install optimum>=1.16.0"
        )
        require_version(
            "auto_gptq>=0.5.0", "To fix: pip install auto_gptq>=0.5.0"
        )
        ...
        logger.info(
            "Quantizing model to {} bit.".format(
        ...
        if (
            ...
            require_version(
                "accelerate>=0.28.0", "To fix: pip install accelerate>=0.28.0"
            )
            require_version(
                "bitsandbytes>=0.43.0",
