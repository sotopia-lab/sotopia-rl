...
def run_rloo(
    ...
    if training_args.do_train:
        rloo_trainer.rloo_train(
            resume_from_checkpoint=training_args.resume_from_checkpoint
        )
        rloo_trainer.save_model()
        if training_args.should_save:
            fix_valuehead_checkpoint(
                model, training_args.output_dir, training_args.save_safetensors
            )
        rloo_trainer.save_state()  # must be called after save_model to have a folder
        if rloo_trainer.is_world_process_zero() and finetuning_args.plot_loss:
