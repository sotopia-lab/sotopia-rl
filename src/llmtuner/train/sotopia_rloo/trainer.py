...
class CustomRLOOTrainer(RLOOTrainer, Trainer):
    ...
    def rloo_train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        r"""
        Implements training loop for the RLOO stage, like _inner_training_loop() in Huggingface's Trainer.
        """
        if resume_from_checkpoint is not None:
            raise ValueError(
                "`resume_from_checkpoint` will be surloorted in the future version."
            )
        ...
        for step in tqdm(
            ...
            loss_meter.update(float(stats["rloo/loss/total"]), n=len(rewards))
            reward_meter.update(
            ...
            if (
                ...
                logs = dict(
                    ...
                    learning_rate=stats["rloo/learning_rate"],
    ...
    def get_inputs(
        ...
        ):  # handle llama2 rloo with gradient accumulation > 1
            start_index = (
    ...
    def get_rewards(
        ...
        if (
            getattr(unwrapped_model.config, "model_type", None) == "chatglm"
        ...
        for i in range(values.size(0)):
            end_indexes = (
                batch["input_ids"][i] != self.tokenizer.pad_token_id
            ).nonzero()
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            if (
                getattr(unwrapped_model.config, "model_type", None)
                == "chatglm"
            ):
                values = torch.transpose(values, 0, 1)
