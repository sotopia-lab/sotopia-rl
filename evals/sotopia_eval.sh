python -m vllm.entrypoints.openai.api_server --model google/gemma-2-2b-it --chat-template gemma-it.jinja
...
'--gin.AGENT1_MODEL="custom/google/gemma-2-2b-it@http://localhost:8000/v1"' \
'--gin.AGENT2_MODEL="gpt-3.5-turbo"' \
'--gin.BATCH_SIZE=1' \
'--gin.PUSH_TO_DB=True' \
'--gin.TAG="gemma-2-2b-it_vs_gpt-3.5-turbo"'



python -m vllm.entrypoints.openai.api_server --model google/gemma-2-2b --chat-template mistral-instruct.jinja --served-model-name sotopia_gemma-2-2b-sft --enable-lora --lora-modules sft=gemma-2-2b-sft
...
python -m vllm.entrypoints.openai.api_server --model google/gemma-2-2b --chat-template mistral-instruct.jinja --served-model-name sotopia_gemma-2-2b-ppo --enable-lora --lora-modules ppo10=gemma-2-2b-ppo/checkpoint-10
